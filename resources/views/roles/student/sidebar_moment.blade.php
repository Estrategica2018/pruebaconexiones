...
@if(isset($moment))
    ...
    <div class="fs--2 mt-3 font-weight-semi-bold">
        ...
        @if($section['section']['type'] == 1)
            <a id="section_type_question" class="@if($section_type == 1) bottom-selected @endif  cursor-pointer d-flex color-gray-dark mt-2" href="{{route('student.show_moment_section',['empresa'=>auth('afiliadoempresa')->user()->company_name(), 'sequence_id' => $sequence_id, 'moment_id' => $moment->id, 'section_id' => ($index+1),'account_service_id'=>$account_service_id,'order_moment_id'=>$order_moment_id])}}">
